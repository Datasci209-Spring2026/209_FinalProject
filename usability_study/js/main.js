...
async function init() {
  const energyData = await d3.csv(
    "usability_study/data/eia_monthly_generation_by_source.csv",
    parseEnergyRow
  );

  const dataState = await d3.csv(
    "usability_study/data/eia_monthly_generation_by_state_by_source.csv",
    parseEnergyRow
  );

  const plantYearData = await d3.csv(
    "usability_study/data/eia_plant_year_dominant_source_2001_2024.csv",
    parsePlantRow
  );
